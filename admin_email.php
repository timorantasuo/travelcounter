<?php
   include('session.php');
  /*** first check that emails have been sent ***/
	if(!isset( $_POST['counter_email'], $_POST['counter_email_new']))
	{
    $message = 'Change your email below.<br><strong>Current email</strong><br><strong>Email: </strong>'.$row["counter_email"].'';
	}
	...
	/*** check the email is valid ***/
	elseif (!filter_var($_POST['counter_email_new'], FILTER_VALIDATE_EMAIL))
	{
		$message = 'Please, enter a new valid email';
		
	}

else{
		$email = mysqli_real_escape_string($db,$_POST['counter_email']);
		$email_new = mysqli_real_escape_string($db,$_POST['counter_email_new']);
		
		
	 if($_SERVER["REQUEST_METHOD"] == "POST") {
      // username and password sent from form 
      
      $sql = "SELECT counter_email FROM counter_users WHERE counter_email = '$user_check'";
      ...
      // If result matched $email , table row must be 1 row
        ...
        /*** prepare the insert ***/
        $statement = "UPDATE counter_users SET counter_email= '$email_new' WHERE counter_email= '$user_check'";
		$retval = mysqli_query($db,$statement);
            
            if(! $retval ) {
               $message ='Could not update email';
            }
            $message = "Succesfully changed email.<br><strong>Email:</strong> ".$email_new.'</strong><br><b>To apply the changes, you need to <a href="logout_message.php">log out and log in again.</b></a>';
          ...
          <div class="col-sm-9 col-md-9 templatemo_col9">
            ...
            <nav class="mainMenu">
              <ul class="nav">
                ...
                <li><a class="menu" href="#home">Email change</a></li>
      	...
      	<div  id="slider"  class="nivoSlider templatemo_slider">
            ...
            <img src="images/slider/img_3_blank.jpg" alt="slide 3" />	        	
             ...
             <div class="templatemo_caption">
                    ...
                    <div class="clear"></div>
                    <h1>Email change to: <?php echo $row["counter_name"]." ".$row["counter_fam_name"]."";?></h1>
                    <div class="clear"></div>
      ...
      <div class="col-md-offset-1 col-md-3 col-xs-8 col-xs-offset-2">
        <form action="" method="post">
          <div class="form-group">
           <input name="counter_email" type="text" class="form-control" id="counter_email" placeholder="Your current e-mail" maxlength="150">
          </div>
          <div class="form-group">
           <input name="counter_email_new" type="text" class="form-control" id="counter_email_new" placeholder="Your new e-mail" maxlength="150">
          </div>
          
          <div>
            <input type="submit" name="submit" value="Change email" class="btn btn-primary"/>
          </div>
        </form>
      </div>
